...
Route::middleware(ResolveTenant::class)->group(function (): void {
    ...
    // Key management (list/mint/revoke) is admin-scoped (PROJECT.md §key-mgmt) and an
    // escalation vector: require the apikeys:manage scope (a '*' key satisfies it; session
    // auth is unaffected). ResolveTenant runs once for the group; the scoped instance here
    // additionally enforces the ability for API keys.
    Route::get('/api-keys', [ApiKeyController::class, 'index'])
        ->middleware(ResolveTenant::class.':apikeys:manage')
        ->name('price-intelligence.api-keys.index');
